...
inline fun <L, R> Observable<Either<L, R>>.onError(crossinline onError: (L) -> Unit): Observable<Either<L, R>> = map {
    mapLeft(it, onError)
}

inline fun <L, R> Single<Either<L, R>>.onSuccess(crossinline onSuccess: (R) -> Unit) = map {
    mapRight(it, onSuccess)
}

inline fun <L, R> Single<Either<L, R>>.onError(crossinline onError: (L) -> Unit) = map {
    mapLeft(it, onError)
}

inline fun <L, R> Maybe<Either<L, R>>.onSuccess(crossinline onSuccess: (R) -> Unit) = map {
    mapRight(it, onSuccess)
}

inline fun <L, R> Maybe<Either<L, R>>.onError(crossinline onError: (L) -> Unit) = map {
    mapLeft(it, onError)
}

inline fun <R> Maybe<Either<LocationError, R>>.onLocationError(crossinline onError: (LocationError) -> Unit) = map { either ->
